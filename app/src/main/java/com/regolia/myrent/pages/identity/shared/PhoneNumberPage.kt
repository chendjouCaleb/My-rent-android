...
@Composable
fun PhoneNumberPage(onValueChange: (value: String) -> Unit) {
    val viewModel = viewModel { PhoneNumberViewModel() }
    Column(Modifier.animateContentSize()) {
        Text(text = "Nous devons vérifier votre numéro de téléphone",
            style = MaterialTheme.typography.bodyMedium)
        Spacer(modifier = Modifier.height(16.dp))


        var country by remember { mutableStateOf("cameroun") }
        FluentOutlinedTextField(value = country, onValueChange = {country = it},
            trailingIcon = { Icon(imageVector = Icons.Default.ArrowDropDown, contentDescription = "") },
            modifier = Modifier.fillMaxWidth())

        Spacer(modifier = Modifier.height(16.dp))
        Row(Modifier.fillMaxWidth()) {
            FluentOutlinedTextField(value = viewModel.indicator, onValueChange = {},
            ...
            FluentOutlinedTextField(
                ...
                placeholder = { Text(text = "N° de téléphone") },
